...
table3$Social.Media = reorder(table3$Social.Media, -tabl31$Freq)
...
xData1 = c( x30, x30,x30,x30,x60,x60,x60,x60,x60,x60,x60,x60,x60,x60,x60, x120,x120,x120,x120,x120,x120,x120,x120,x120,x120,x120,x120,x120,x120,x120,x120)

#combine mean values of sleep hours for each social media usage time range category into vector
meanData2 = c(7,7,6,8,6,7,7,6,7,6,7,10,6,6,6,6,8,7,5,8,8,6,7,6,6,7,7,6,6,5,5)
...
x1 = table5$`Amt of Sleep`
x2 = table5$`SM use in hours`
xy = data.frame(x1,x2)

ggplot(xy, aes(x1, x2, group = x1)) + geom_boxplot() + theme_classic() +
  labs( y="Amount of social media usage daily", x= "Amount of sleep hours", title = "Association between students amount of social media usage and amount of sleep")
...
#--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
 ...
 table9 =  cbind("Amt of time on sm before bed" = jData2, "Amt of sleep"=jData1)
  ...
  xx = table9$Amt.of.sleep
  
yy = table9$Amt.of.time.on.sm.before.bed

xxyy = data.frame(xx, yy)
ggplot(xxyy, aes(xx, yy, group = xx)) + geom_boxplot() + theme_classic() +
  labs( y ="Amount of social media usage before bed", x = "Amount of sleep hours", title = "Association between students' amount of sleep and amount of social media usage before bed ")
...
example$timeslot = factor(c( "After 8 PM but before 9 PM", "After 10 PM but before 11 PM", "After 11 PM but before 12 AM","After 12 AM but before 1 AM", "After 1 AM but before 2 AM", "After 2 AM"), levels = c("After 8 PM but before 9 PM", "After 10 PM but before 11 PM", "After 11 PM but before 12 AM","After 12 AM but before 1 AM", "After 1 AM but before 2 AM", "After 2 AM"))
modes.sleep.time = rename(modes.sleep.time, Negative.sleep.time = "mode.sleep.time2")

a = c("Positive Impact","Positive Impact","Positive Impact","Positive Impact","Positive Impact", "Positive Impact", "Negative Impact", "Negative Impact", "Negative Impact", "Negative Impact", "Negative Impact", "Negative Impact")
b = c("After 8 PM but before 9 PM", "After 10 PM but before 11 PM", "After 11 PM but before 12 AM","After 12 AM but before 1 AM", "After 1 AM but before 2 AM", "After 2 AM")
c = c(1,0,5,4,3,4,0,1,2,4,7,2)
example = data.frame(Impact = a, timeslot = b, freq = c)

ggplot(example, aes(x = example$timeslot, y = example$freq, fill = example$Impact)) + geom_col()+
  labs(x = "Sleep time", y = "Frequency", title = "Positive and Negative impacted students' frequency of time they sleep")
